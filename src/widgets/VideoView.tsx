...
import React, {Component, ElementRef, Ref} from 'react';
// @ts-ignore
...
function calculateProgress({currentTime, duration}: { currentTime: number; duration: number }) {
	return (100 / duration) * currentTime;
}

const Footer = ({interactionIsIdle, children}: { interactionIsIdle: boolean, children: JSX.Element }) => (
	<div
		style={{
			alignItems: 'center',
			bottom: 0,
		...
		return (
			...
			>
				<video
					autoPlay={false}
					controls={false}
					onClick={(a) => this.playOrPause('play')}
					poster={data.poster}
					ref={this.getVideo}
					style={{width: '100%', height: 'auto'}}
				>
					{this.props.data.urls.map((vid, idx) => (
